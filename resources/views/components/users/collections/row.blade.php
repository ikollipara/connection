...
<tr class="is-clickable" x-data
  ...
  <td class="buttons mb-0">
    @if ($item->published)
      <a href="{{ route('collections.show', $item) }}" class="button is-primary">
        <x-lucide-arrow-right class="icon" width="30" height="30" fill="none" />
      </a>
    @endif
    <a class="button is-primary is-outlined" href="{{ route('users.collections.edit', ['me', $item]) }}">
      <x-lucide-pencil class="icon" width="30" height="30" fill="none" />
    </a>
    <form action="{{ route('users.collections.update', ['me', $item]) }}" method="post">
      @csrf
      @method('PATCH')
