...
<span x-data="{ show: false }">
  <button type="button"
          ...
          x-on:click.stop="show = true">{{ $btnDelete ? '' : $btn }}</button>
  <section {{ $attributes->class(['modal']) }}
           x-bind:class="{ 'is-active': show }">
    <div class="modal-background"
         x-on:click.stop="show = false"></div>
    <article class="modal-card">
      <header class="modal-card-head">
        ...
        @unless ($noDelete)
          <button class="delete"
                  type="button"
                  x-on:click.stop="show = false"></button>
        @endunless
      </header>
