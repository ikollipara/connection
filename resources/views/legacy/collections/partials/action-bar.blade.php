{{--
file: resources/views/collections/partials/action-bar.blade.php
...
<ul class="flex flex-wrap items-center justify-start text-gray-900 dark:text-white">
  <li>
    <x-form x-data
      ...
      <button class="me-4 hover:underline md:me-6"
              type="submit"
              x-ref="like_button">{{ $collection->likes() }} {{ Str::plural('Like', $collection->likes()) }}</button>
    </x-form>
  </li>
  <li>
    <p class="me-4  md:me-6 ">{{ $collection->views() }} Views</p>
  </li>
  @auth
    <li>
      <button class="me-4 hover:underline md:me-6"
  ...
  <li>
    <a class="me-4 hover:underline md:me-6"
       href="{{ route('users.profile.show', $collection->user) }}">View Author</a>
  </li>
</ul>
@push('modals')
  <div class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full"
    ...
    <div class="relative p-4 w-full max-w-2xl max-h-full"
         x-data="{ collections: [], collectionId: {{ Js::from($collection->id) }} }"
         x-init="fetch('{{ route('api.users.collections.index', auth()->user()) }}?content_id=' + collectionId)
             .then(response => response.json())
             .then(data => collections = data.collections.map(el => ({ ...el, selected: Boolean(el.hasEntry) })).filter(el => el.id !== '{{ $collection->id }}'))">
      <!-- Modal content -->
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
          <button class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                  ...
                  $nextTick(() => {
                    fetch(
                        '{{ route('api.collections.collections.store', $collection) }}',
                        {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                        ...
                        if (!response.ok) {
                            ...
                            alert('Collection added to collection');
                        }
                    });
                  })
                  ">Save</button>
