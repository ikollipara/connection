{{--
file: resources/views/users/collections/partials/publishing-drawer.blade.php
author: Ian Kollipara
date: 2024-09-09
description: The drawer for publishing a collection
 --}}
...
<div class="fixed top-0 left-0 z-40 w-96 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-white dark:bg-gray-800"
  ...
  <h5 class="text-base font-semibold text-gray-500 uppercase dark:text-gray-400"
      id="{{ $name }}-label">Collection Details</h5>
    ...
    <ul class="space-y-2 font-medium">
      <li>
        <x-form-select name="grades"
                       :options="Grade::cases()"
                       :selected="$collection->metadata->grades"
                       multiple />
      </li>
      <li>
        <x-form-select name="audience"
                       :selected="collect($collection->metadata->audience)"
                       :options="Audience::cases()" />
      </li>
      <li>
        <x-form-select name="languages"
                       :options="Language::cases()"
                       :selected="$collection->metadata->languages"
                       multiple />
      </li>
      <li>
        <x-form-select name="category"
                       :selected="collect($collection->metadata->category)"
                       :options="Category::cases()" />
      </li>
      <li>
        <x-form-select name="practices"
                       :selected="$collection->metadata->practices"
                       :options="Practice::cases()"
                       multiple />
      </li>
      <li>
        <x-form-submit label="Publish Collection" />
      </li>
    </ul>
  </x-form>
</div>
